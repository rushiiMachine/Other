...
class playThread(threading.Thread):
	def __init__(self, threadID, name):
		...
		self.name = name
	def run(self):
		playsound('shriek.mp3')

thread3 = relayThread2(3, "RelayThread-3")
thread4 = playThread(4,"RelayThread-4")
...
while True:
	...
	if text=="Occupied":
		if datetime.now() > (timeForRelay + timedelta(seconds=40)): # Interval between when it can be activated; change `seconds=40`
			emitFor6Sec()
	else:
		if datetime.now() > firstTime  + timedelta(minutes=1):
