...
secsWithSmoke = 0
timeForRelay = datetime.datetime.now() - datetime.timedelta(minutes=1)
...
def emitFor6Sec():
    ...
    thread.start()
    global timeForRelay
    timeForRelay = datetime.datetime.now()
...
while True:
    ...
    if text == "Occupied":
        if datetime.datetime.now() > (timeForRelay + datetime.timedelta(seconds=30)):
            emitFor6Sec()

    t.sleep(0.1)

    # Check if 10 minutes has passed since last firstFrame, if so put a new frame if unoccupied
    # if firstTime + datetime.timedelta(minutes=5) <= datetime.datetime.now():
    # 	if text == "Unoccupied":
    # 		if secsWithoutSmoke > 100:
    # 			firstFrame = frame
